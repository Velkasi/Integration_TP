...
dataframeEnrollments = pd.read_csv("./jeux de données et bdd/enrollments.csv")
dataframeMajors = pd.read_csv("TP_Migration_Old_To_New/TD reprise de données/jeux de données et bdd/majors.csv")
dataframeUniversity = pd.read_csv("TP_Migration_Old_To_New/TD reprise de données/jeux de données et bdd/university_db.csv")
...
for _, row in dataframeEnrollments.iterrows():
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO employees (major_code, registration_date, status, student_id)
            VALUES (%s, %s, %s, %s, %s)
            """, (
